...
**NOTE:** Example setting files for the bitcoin, dogecoin and bltg networks are also provided. Copy or rename `settings.conf.bitcoin`, `settings.conf.dogecoin` or `settings.conf.bltg` to `settings.conf` and test crawling any of these networks with working examples to better understand how to set up the config for your own network.
